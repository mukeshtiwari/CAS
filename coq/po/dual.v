...
Lemma brel_dual_reflexive (lteRefl : brel_reflexive S lte) : brel_reflexive S (brel_dual lte). 
Proof. intros s; compute; auto. Qed. 

Lemma brel_dual_not_reflexive (NlteRefl : brel_not_reflexive S lte) : brel_not_reflexive S (brel_dual lte). 
Proof. destruct NlteRefl as [s P]. exists s; compute; auto. Defined.

Definition brel_dual_reflexive_decide : 
     brel_reflexive_decidable S lte  → brel_reflexive_decidable S (brel_dual lte)
:= λ dS,  
   match dS with 
...
Lemma brel_dual_transitive (lteTran : brel_transitive S lte) : brel_transitive S (brel_dual lte).
Proof. intros s1 s2 s3; compute; intros A B; auto. exact (lteTran _ _ _ B A). Qed.

Lemma brel_dual_antisymmetric (anti : brel_antisymmetric S eq lte): brel_antisymmetric S eq (brel_dual lte).
Proof. intros s1 s2; compute; intros A B; auto. Qed.

Lemma brel_dual_not_antisymmetric (nanti : brel_not_antisymmetric S eq lte): brel_not_antisymmetric S eq (brel_dual lte).
Proof. destruct nanti as [[s1 s2] [A B]]. exists (s1, s2); compute; split; auto.
       destruct A as [A C]. split; auto.
Defined.

Definition brel_add_bottom_antisymmetic_decide (dS : brel_antisymmetric_decidable S eq lte) : brel_antisymmetric_decidable S eq (brel_dual lte)
:= match dS with 
...
Lemma brel_dual_total (tot : brel_total S lte) : brel_total S (brel_dual lte).
Proof. intros s1 s2; compute; auto. Qed. 

Lemma brel_dual_not_total (ntot : brel_not_total S lte) : brel_not_total S (brel_dual lte).
Proof. destruct ntot as [[s1 s2] [A B]]. exists (s1, s2). compute; auto. Defined. 

Definition brel_dual_total_decide (dS : brel_total_decidable S lte) : brel_total_decidable S (brel_dual lte)
:= match dS with 
...
(*
Lemma brel_add_bottom_exists_bottom (topS : brel_exists_top S eq lte) : brel_exists_bottom S eq (brel_dual lte)
Proof. destruct topS as [top P]. exists (inl bottom). apply brel_add_bottom_is_bottom.
