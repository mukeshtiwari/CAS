...
Section Theory.
 ...
 Lemma ltr_from_sg_congruence : ltr_congruence S S eqS eqS (ltr_from_sg bS).
 Proof. unfold ltr_congruence. unfold ltr_from_sg. intros. apply bS_cong; auto. Qed.

(* 
 Lemma ltr_from_sg_partial_congruence : ltr_partial_congruence S S eqS (ltr_from_sg bS). 
 Proof. unfold ltr_congruence. intros s s1 s2 H.
        unfold ltr_from_sg. apply bS_cong; auto. 
 Qed.
 *)
 
 Lemma ltr_from_sg_is_right : bop_is_right S eqS bS -> ltr_is_right S S eqS (ltr_from_sg bS).
 Proof. auto. Qed. 
 
 Lemma ltr_from_sg_not_is_right : bop_not_is_right S eqS bS -> ltr_not_is_right S S eqS (ltr_from_sg bS).
 Proof. auto. Defined.
 ...
 Lemma ltr_from_sg_cancellative : bop_left_cancellative S eqS bS -> ltr_left_cancellative S S eqS (ltr_from_sg bS).
 Proof. auto. Qed.

 Lemma ltr_from_sg_not_cancellative : bop_not_left_cancellative S eqS bS -> ltr_not_left_cancellative S S eqS (ltr_from_sg bS).
 Proof. auto. Defined. 
 
 Lemma ltr_from_sg_exists_id : bop_exists_left_id S eqS bS -> ltr_exists_id S S eqS (ltr_from_sg bS).
 Proof. auto. Qed. 

 Lemma ltr_from_sg_not_exists_id : bop_not_exists_left_id S eqS bS -> ltr_not_exists_id S S eqS (ltr_from_sg bS).
 Proof. auto. Defined. 
...
Definition ltr_from_sg_proofs (S : Type) (eqS : brel S) (bS : binary_op S) (msgP : sg_proofs S eqS bS) :
    left_transform_proofs S S eqS eqS (ltr_from_sg bS) := 
{|
  A_left_transform_congruence   := ltr_from_sg_congruence S eqS bS (A_sg_congruence _ _ _ msgP)
; A_left_transform_is_right_d   := match A_sg_is_right_d _ _ _ msgP with
                                   | inl IR => inl (ltr_from_sg_is_right _ _ _ IR)
                                   | inr NIR => inr (ltr_from_sg_not_is_right _ _ _ NIR)
                                   end
...
; A_left_transform_left_cancellative_d :=
                                   ...
                                   | inl LC => inl (ltr_from_sg_cancellative _ _ _ LC)
                                   | inr NLC => inr (ltr_from_sg_not_cancellative _ _ _ NLC)
                                   end 
|}.
...
Definition ltr_from_sg_certs (S : Type) (msgP : @sg_certificates S) : @left_transform_certificates S S := 
{|
