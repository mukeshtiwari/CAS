...
Section Matrix.
  Variables 
    ...
    *)
    ...
    (* I need to prove that we are not adding any empty list in 
      the list, generated by all_paths_klength m k c d
      k = 0 
    ...
    (* I need eqR on list of lists? *)
    Lemma append_node_app : forall (l₁ l₂ : list (list (Node * Node * R))) 
      (m : Matrix) (c : Node), 
      append_node_in_paths m c (l₁ ++ l₂) = 
      append_node_in_paths m c l₁ ++ append_node_in_paths m c l₂.
    Proof.
    ...
    (* This is just here for the moment, it will go away once 
      we have all the requiv proofs for rewriting *)
    Lemma fold_right_dist : forall l₁ l₂ : list R, 
      fold_right (fun u v : R => u + v) 0 (l₁ ++ l₂) =
      fold_right (fun u₁ v₁ : R => u₁ + v₁) 0 l₁ + 
      fold_right (fun u₂ v₂ : R => u₂ + v₂) 0 l₂.
    Proof.
    ...
    Proof.
      ...
      - simpl; intros ? ? ? ?.
        ...
        rewrite append_node_app.
        rewrite map_app.
        rewrite fold_right_dist.
        apply congrP. apply fold_right_measure.
        apply IHL.
    Admitted.
