...
Lemma bop_product_llex_not_left_distributive_v555 (t1 t2 t3 : T)
      ...
      (ldT : bop_left_distributive T rT mulT addT) : 
         bop_not_left_distributive (S * T) (rS <*> rT) (mulS [*] mulT) (addS [+] addT).
Proof. destruct nlcS as [[s1 [s2 s3]] [H1 H2]]. 
       exists ((s1, t1), ((s2, t2), (s3, t3))). compute.
       rewrite ldS.
       destruct (selS s1 s2) as [A | A]; 
       destruct (selS s1 s3) as [B | B].
       + apply symS in A, B.
         rewrite A, B.
         admit. 
       + admit.
       + admit.
       + rewrite A, B.
         assert (C : rS (s1 +S (s2 *S s3)) (s2 *S s3) = true). admit.
         rewrite C.
         admit. 
Admitted. 
...
Lemma bop_product_llex_not_left_distributive_v66 (t1 t2 t3 : T)
      ...
      (ldT : bop_left_distributive T rT mulT addT) : 
         bop_not_left_distributive (S * T) (rS <*> rT) (mulS [*] mulT) (addS [+] addT).
Proof. destruct m_nidemS as [s H]. 
       exists ((s, t1), ((s, t2), (s, t3))). compute.
       rewrite ldS.
       assert (A := a_idemS s). rewrite A. apply symS in A. rewrite A.
       assert (B := ldS s s s).
       assert (C := m_conS _ _ _ _ A A). 
       assert (D := tranS _ _ _ B (symS _ _ C)). rewrite D. 
       case_eq(rS s (s +S (s *S s))); intro E.
       + assert (F := tranS _ _ _ E D). apply symS in F.
         rewrite F in H. discriminate H.
       + assert (F := ldT t1 t2 t3).
         admit. (* ??????????? *) 
Admitted. 
