...
Require Import CAS.coq.po.strictify. 
   ...
   match x, y with
    | (a, b), (c, d) => 
        (b1 a c, 
         if eq a c 
         then (b2 b d)
         else if brel_lt_left eq b1 a c 
              then b 
              else d)
   end.

Definition bop_gllex : ∀ {S T : Type}, T → brel S → binary_op S → binary_op T → binary_op (S * T) 
    ...
    | (a, b), (c, d) => 
        (b1 a c, 
         ...
         then (b2 b d)
              ...
              then b 
              else if brel_lt_left eq b1 c a 
                   then d
                   else t)
   end.
...
Lemma bop_llex_congruence (selS : bop_selective S rS bS) : bop_congruence (S * T) (rS <*> rT) (bS [*] bT). 
Proof. unfold bop_congruence.  
       ...
       unfold brel_product. unfold bop_llex. rewrite qS. 
       case_eq(rS s1 s2); intro D; rewrite <- hS; rewrite D.
          simpl. exact qT.
          assert (E : brel_lt_left rS bS s1 s2 = brel_lt_left rS bS s3 s4).
             compute. rewrite hS2.
             case_eq(rS s3 (s3 *S s4)); intro E; auto. 
                assert (F : rS s2 (s2 *S s1) = rS s4 (s4 *S s3)).
                   exact(conS _ _ _ _ C3 (b_conS _ _ _ _ C3 C1)). 
                rewrite <- F. 
                case_eq(rS s2 (s2 *S s1)); intro G; reflexivity. 
             
          rewrite <- E. 
          case_eq(brel_lt_left rS bS s1 s2); intro F. 
                simpl. exact C2. 
                compute in F. destruct (selS s1 s2) as [G | G].
                   apply symS in G. rewrite G in F. 
                   case_eq(rS s2 (s2 *S s1)); intro I. 
                      simpl. exact C4. 
...
Proof. intros commS iS iT pS pT [s t].  
       ...
       case_eq(rS iS s); intro Q. 
          ...
          unfold brel_lt_left. unfold brel_lte_left, brel_strictify.  
          rewrite (brel_symmetric_implies_dual _ _ symS _ _ Q). 
...
Proof. intros [ [t t'] P]. exists ((wS, t), (wS, t')). simpl. apply bop_and_false_intro. right. rewrite (refS wS). assumption. Defined. 
...
Proof. intros commS selS selT [s1 t1] [s2 t2]. 
    assert (idemS := bop_selective_implies_idempotent S rS bS selS). 
    unfold brel_product, bop_llex. unfold brel_lt_left, brel_lte_left, brel_strictify. 
    destruct (selS s1 s2) as [H | H]; destruct (selT t1 t2) as [Q | Q]. 
      ...
      case_eq(rS s1 s2); intro J. 
         exact Q. 
         compute. case_eq(rS s2 (s2 *S s1)); intro K. 
            assert (I := commS s1 s2). 
            assert (L := tranS _ _ _ H I). apply symS in K. 
            assert (M := tranS _ _ _ L K). rewrite M in J. discriminate J. 
      ...
      case_eq(rS s1 s2); intro J. 
         ...
         left.  rewrite H. simpl. apply symS in H. 
                ...
                compute. 
                case_eq(rS s2 (s2 *S s1)); intro K. 
                   assert (I := commS s2 s1). 
                   assert (L := tranS _ _ _ K I). apply symS in H.
                   assert (M := tranS _ _ _ L H). rewrite M in J. discriminate J. 
   ...
   match x, y with
    | (a, b), (c, d) => 
        (b1 a c, 
         if eq a c 
         then (b2 b d)
         else if brel_lt_left _ eq b1 a c 
              then b 
              else d)
   end.
...
Proof. intros s1 s2 s3 commS selS A1 notB1 B2. 
       destruct (brel_lt_left_elim S rS bS s1 s2 B2) as [A C]. 
       assert (C1 : s1 =S (s2 *S s3)). 
       ...
       assert (B := tranS _ _ _ A A1). apply symS in B. 
       rewrite (conS _ _ _ _ (refS s2) B). 
       apply (brel_symmetric_implies_dual S rS symS) in notB1.
       exact notB1. 

       apply brel_lt_left_false_intro. left. 
...
Proof. intros commS selS s1 s2 H. 
       ...
       destruct H as [H | H]; destruct (selS s2 s1) as [J | J]. 
         ...
         apply symS. assumption. 
         apply symS in H. apply symS in J. exact (symS _ _ H). 
Defined. 
...
Proof. 
       ...
       assert (fact1 := brel_lt_left_false_elim2 commS selS _ _ notB2). 
       assert (fact2 := commS s2 s1). 
       unfold brel_lte_left in fact1. 
          ...
          bop_commutative S rS bS → bop_selective S rS bS → 
           ∀ (s1 s2 s3 : S), 
             ...
             (* B1 *)           s1 =S s2 ->  
                 ...
                 (* !D1 *)        (s2 !=S s3) * 
                 (* D2  *)         (s2 << s3). 
Proof. intros commS selS s1 s2 s3 notA1 A2 B1. 
       assert (idemS := bop_selective_implies_idempotent _ _ _ selS).
       assert (I := idemS s2). 
       assert (D2 : s2 << s3). 
          assert (A := b_conS _ _ _ _ B1 (refS s2)). 
          assert (B := tranS _ _ _ A I).
          rewrite (brel_lt_left_congruence S rS conS bS b_conS _ _ _ _ B (refS s3)) in A2. 
          assumption. 
       destruct (brel_lt_left_elim S rS bS _ _ D2) as [A notD1].
       assert (C1    : s1 =S (s2 *S s3)). 
          rewrite (conS _ _ _ _ B1 (refS (s2 *S s3))). 
          assumption. 
       rewrite C1, D2.  apply brel_lt_left_elim in D2. destruct D2 as [D2 D3]. 
       case_eq(rS s2 s3); intro K; auto. 
          assert (J := commS s2 s3). apply symS in K. 
          assert (L := tranS _ _ _ K D2).
          assert (M := tranS _ _ _ L J).
          rewrite M in D3. discriminate D3. 
Defined. 
...
!B1 : s1 <> s2
 ...
 C2 : B2, A2 
     -> s1 < s3
     -> s1 < (s2 + s3) brel_lt_left_compatible_right

!C1 : brel_lt_left_elim1 C2 
     -> s1 <> (s2 + s3) 
...
Proof. intros s1 s2 s3 commS selS notA1 A2 notB1 B2. 
       ...
       apply brel_lt_left_elim in A2. destruct A2 as [AL AR].
       apply brel_lt_left_elim in B2. destruct B2 as [BL BR].       
       ...
       destruct (selS s2 s3) as [Q | Q]; split. 
          ...
          apply brel_lt_left_intro. 
             exact fact0. 
             case_eq(rS (s2 *S s3) ((s2 *S s3) *S s1)); intro K; auto. 
                assert (J := commS s1 (s2 *S s3)). 
                assert (I := tranS _ _ _ fact0 J). apply symS in K. 
                assert (L := tranS _ _ _ I K).
                assert (M := tranS _ _ _ L Q).
                rewrite M in notB1. discriminate notB1. 
          ...
          apply brel_lt_left_intro.
             exact fact0.
             (* I'm sure there is a shorter proof! *)
             case_eq(rS (s2 *S s3) ((s2 *S s3) *S s1)); intro K; auto. 
                assert (J := commS (s2 *S s3) s1). apply symS in Q. 
                assert (I := tranS _ _ _ Q K). 
                assert (L := tranS _ _ _ I J).
                assert (M := assS s1 s2 s3). apply symS in M. 
                assert (N := tranS _ _ _ L M). apply symS in AL.
                assert (O := tranS _ _ _ N AL). apply symS in O.                 
                rewrite O in notA1. discriminate notA1. 
Defined. 
...
Lemma lex_lemma_case_6 : 
          bop_commutative S rS bS → bop_selective S rS bS → 
           ...
           (* !B2 *)         s1 !<< s2 -> 
               ...
               (* !D1 *)          (s2 !=S s3) *
               (* D2  *)           (s2 << s3). 
Proof. intros commS selS s1 s2 s3 notA1 A2 notB1 notB2.  
       apply brel_lt_left_false_elim in notB2. destruct notB2 as [notB2  | notB2].
          apply brel_lt_left_elim in A2. destruct A2 as [L R].
          ...
          split. split. split.
             ...
             case_eq(rS s3 (s3 *S s2)); intro K; auto. 
                assert (J := b_conS _ _ _ _ (refS s3) fact1). 
                assert (I := tranS _ _ _ K J).                        
                assert (M := commS s3 (s1 *S s2)). 
                assert (N := tranS _ _ _ I M). apply symS in L. 
                assert (O := tranS _ _ _ N L). apply symS in O. 
                assert (P := tranS _ _ _ fact1 O). rewrite P in notD1. discriminate notD1.

       (* perhaps I should start using tactics ;-)  *) 
       apply brel_lt_left_elim in A2. destruct A2 as [A2 A3]. 
       assert (K := commS s2 s1).
       assert (J := tranS _ _ _ notB2 K). 
       assert (I := tranS _ _ _ J A2). 
       assert (L := b_conS _ _ _ _ J (refS s3)). apply symS in L. 
       assert (M := tranS _ _ _ I L).      
       case_eq(rS s1 (s2 *S s3)); intro N; auto. 
         apply symS in M. rewrite (tranS _ _ _ N M) in notB1. discriminate notB1. 
       case_eq(brel_lt_left rS bS s1 (s2 *S s3)); intro O; auto. 
         apply brel_lt_left_elim in O. destruct O as [O1 O2]. 
         assert (P := assS s1 s2 s3). apply symS in P. 
         assert (Q := tranS _ _ _ O1 P). 
         assert (R := tranS _ _ _ Q L).
         rewrite R in N. discriminate N. 
       case_eq(rS s2 s3); intro P; auto. 
          rewrite (conS _ _ _ _ J (refS s3)) in P. 
          rewrite P in notA1. discriminate notA1. 
       case_eq(brel_lt_left rS bS s2 s3); intro ; auto. 
          apply brel_lt_left_false_elim in H. 
          destruct H as [H | H]. 
             rewrite H in M. discriminate M. 
             assert (Q := commS s3 s2). 
             assert (R := tranS _ _ _ H Q). apply symS in R.
             rewrite (tranS _ _ _ M R) in P. discriminate P. 
Defined.  
...
Proof. intros s1 s2 s3 selS notA1 A3 B1. 
       ...
       assert (D3 : s2 !<< s3). 
          rewrite (brel_lt_left_congruence S rS conS bS b_conS _ _ _ _ C (refS s3)) in A3. 
          assumption. 
       assert (notD1 : s2 !=S s3).
          apply (brel_transititivity_implies_dual _ _ tranS _ _ _ C notA1). 
            assert (C3 : s1 !<< (s2 *S s3)). 
         destruct (selS s2 s3) as [D | D]. 
            rewrite (brel_lt_left_congruence S rS conS bS b_conS _ _ _ _ B1 D).
            rewrite (brel_lt_left_irreflexive S rS symS bS idemS). reflexivity. 

            rewrite (brel_lt_left_congruence S rS conS bS b_conS _ _ _ _ B1 D).
            assumption.
       assert (notC1 : s1 !=S (s2 *S s3)). 
         destruct (brel_lt_left_false_elim _ _ _ _ _ D3) as [D | D]. 
            rewrite(conS _ _ _ _ B1 (refS (s2 *S s3))). assumption.
            apply brel_lt_left_false_elim in D3. 
            apply brel_lt_left_false_elim in C3.
            case_eq(rS s1 (s2 *S s3)); intro E; auto.             
               destruct D3 as [D3 | D3]; destruct (selS s2 s3) as [F | F]; destruct C3 as [C3 | C3].
                  apply symS in F. rewrite F in D3. discriminate D3. 
...
Proof. intros s1 s2 s3 selS notA1 A3 notB1 B2. 
       apply brel_lt_left_false_elim in A3. 
       apply brel_lt_left_elim in B2.  destruct B2 as [fact1 _]. apply symS in fact1. 
...
(*                apply symS in fact1. assert (fact3 := tranS _ _ _ fact1 A3). 
          ...
          assert (notD1 : s2 !=S s3). 
              case_eq(rS s2 s3); intro J; destruct A3 as [A3 | A3]. 
                 ...
                 admit. (*rewrite A3 in notA1. discriminate.                  *)
...
Proof. intros selS commS s1 s2 s3 notA1 notA2 notB1 notB2. 
       apply brel_lt_left_false_elim in notA2. 
       apply brel_lt_left_false_elim in notB2. 
       destruct notA2 as [H | H]; destruct notB2 as [K | K].
         ...
         admit. admit. admit. (* rewrite K in notB1. discriminate.  
...
Proof. 
    ...
    apply bop_and_intro.
       apply assS.
       (* 
          [(s1 =S s2 * s1 !<< s2)  +  (s1 !=S s2 * s1 << s2)  +  (s1 !=S s2 * s1 !<< s2)]
          * 
          [((s1 *S s2) =S s3 * (s1 *S s2) !<< s3)  +  ((s1 *S s2) !=S s3 * (s1 *S s2) << s3)  +  ((s1 *S s2) !=S s3 * (s1 *S s2) !<< s3)] 
       *) 
       destruct (brel_lt_left_total_order_split S rS refS symS tranS bS b_conS idemS commS selS (bS s1 s2) s3) as [[[Aeq Alt] | [Aeq Alt]] | [Aeq Alt]]; 
       destruct (brel_lt_left_total_order_split S rS refS symS tranS bS b_conS idemS commS selS s1 s2)         as [[[Beq Blt] | [Beq Blt]] | [Beq Blt]]. 
