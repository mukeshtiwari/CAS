...
Lemma brel_product_negate : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              (brel_negate S rS) → (brel_negate T rT) 
               → brel_negate (S * T) (brel_product S T rS rT). 
Proof. 
     intros S T rS rT [fS PS] [fT PT]. 
...
Definition brel_product_nontrivial : ∀ (S T : Type) (rS : brel S) (rT : brel T), 
       (brel_nontrivial S rS) → (brel_nontrivial T rT) → 
         brel_nontrivial (S * T) (brel_product S T rS rT)
:= λ S T rS rT ntS ntT, 
   {| 
      brel_nontrivial_witness  := brel_product_witness S T rS rT 
                                     (brel_nontrivial_witness S rS ntS)
                                     (brel_nontrivial_witness T rT ntT)
    ; brel_nontrivial_negate   := brel_product_negate S T rS rT 
...
Lemma brel_product_congruence : 
         ∀ (S T: Type) (eq1 : brel S) (eq2 : brel T) (r1 : brel S) (r2 : brel T),  
            brel_congruence S eq1 r1  → 
            brel_congruence T eq2 r2  → 
               brel_congruence (S * T) (brel_product S T eq1 eq2) (brel_product S T r1 r2). 
Proof. unfold brel_congruence. intros S T eq1 eq2 r1 r2 C1 C2. 
...
Lemma brel_product_reflexive : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              (brel_reflexive _ rS) → (brel_reflexive _ rT) 
               → brel_reflexive (S * T) (brel_product S T rS rT). 
Proof. 
     intros S T rS rT. unfold brel_reflexive. intros RS RT [s t].
     simpl. rewrite (RS s), (RT t). simpl. reflexivity. 
Defined. 


Lemma brel_product_not_reflexive_left : ∀ (S T: Type) (rS : brel S) (rT : brel T) (t : T),   
              (brel_not_reflexive _ rS) 
               → brel_not_reflexive (S * T) (brel_product S T rS rT). 
Proof. unfold brel_not_reflexive. intros S T rS rT t [s P]. 
        exists (s, t). compute. rewrite P. reflexivity. 
Defined. 

Lemma brel_product_not_reflexive_right : ∀ (S T: Type) (rS : brel S) (rT : brel T) (s : S),   
              (brel_not_reflexive _ rT) 
               → brel_not_reflexive (S * T) (brel_product S T rS rT). 
Proof. unfold brel_not_reflexive. intros S T rS rT s [t P]. 
        exists (s, t). compute. rewrite P. case_eq(rS s s); intro H; auto. 
Defined. 

Definition brel_product_reflexive_decide: 
   ∀ (S T: Type) (rS : brel S) (rT : brel T) (s : S) (t : T),    
     brel_reflexive_decidable S rS → 
     brel_reflexive_decidable T rT → 
        brel_reflexive_decidable (S * T) (brel_product S T rS rT)
:= λ S T rS rT s t dS dT,  
       match dS with 
       | inl refS => 
         match dT with 
         | inl refT     => inl _ (brel_product_reflexive S T rS rT refS refT)
         | inr not_refT => inr _ (brel_product_not_reflexive_right S T rS rT s not_refT)
         end 
       | inr not_refS   => inr _ (brel_product_not_reflexive_left S T rS rT t not_refS)
       end. 

Lemma brel_product_irreflexive_left : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
        brel_irreflexive S rS -> 
           brel_irreflexive (S * T) (brel_product S T rS rT). 
Proof. 
     unfold brel_irreflexive. intros S T rS rT irr [s t]. compute. 
     rewrite (irr s). reflexivity. 
Defined. 


Lemma brel_product_irreflexive_right : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
        brel_irreflexive T rT -> 
           brel_irreflexive (S * T) (brel_product S T rS rT). 
Proof. 
     unfold brel_irreflexive. intros S T rS rT irr [s t]. compute. 
     rewrite (irr t). case_eq(rS s s); intro H; auto.
Defined. 



Lemma brel_product_not_irreflexive : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
        brel_not_irreflexive S rS -> 
        brel_not_irreflexive T rT -> 
           brel_not_irreflexive (S * T) (brel_product S T rS rT). 
Proof. 
     unfold brel_not_irreflexive. intros S T rS rT [s P] [t Q]. 
     exists (s, t). compute. rewrite P, Q; auto. 
Defined. 
...
Definition brel_product_irreflexive_decide: 
   ∀ (S T: Type) 
     ...
     brel_irreflexive_decidable T rT → 
        brel_irreflexive_decidable (S * T) (brel_product S T rS rT)
:= λ S T rS rT dS dT,  
       match dS with 
       | inl irrS => inl _ (brel_product_irreflexive_left S T rS rT irrS)
       | inr not_irrS   => 
         match dT with 
         | inl irrT     => inl _ (brel_product_irreflexive_right S T rS rT irrT)
         | inr not_irrT => inr _ (brel_product_not_irreflexive S T rS rT not_irrS not_irrT)
         end 
       end. 



Lemma brel_product_symmetric : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              (brel_symmetric _ rS) → (brel_symmetric _ rT) 
               → brel_symmetric (S * T) (brel_product S T rS rT). 
Proof. 
     intros S T rS rT. unfold brel_symmetric. intros RS RT [s1 t1] [s2 t2]; simpl. 
     intros.           
...
Lemma brel_product_not_symmetric_left : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
      brel_not_symmetric S rS → 
      brel_witness T rT → 
         brel_not_symmetric (S * T) (brel_product S T rS rT). 
Proof. unfold brel_not_symmetric. 
       intros S T rS rT [ [s1 s2] [P1 P2]] [t Q]. 
       exists ((s1, t), (s2, t)). compute. 
       rewrite P1, P2, Q. auto. 
Defined. 


Lemma brel_product_not_symmetric_right : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
      brel_not_symmetric T rT → 
      brel_witness S rS → 
         brel_not_symmetric (S * T) (brel_product S T rS rT). 
Proof. unfold brel_not_symmetric. 
       intros S T rS rT [ [t1 t2] [P1 P2]] [s Q]. 
       exists ((s, t1), (s, t2)). compute. rewrite P1, P2, Q. auto. 
Defined. 
...
Definition brel_product_symmetric_decide: 
   ∀ (S T: Type) 
     ...
     brel_symmetric_decidable S rS → 
     brel_symmetric_decidable T rT → 
        brel_symmetric_decidable (S * T) (brel_product S T rS rT)
:= λ S T rS rT ntS ntT dS dT,  
   let wS := brel_nontrivial_witness S rS ntS in 
   let wT := brel_nontrivial_witness T rT ntT in 
       match dS with 
       | inl symS => 
         match dT with 
         | inl symT     => inl _ (brel_product_symmetric S T rS rT symS symT)
         | inr not_symT => inr _ (brel_product_not_symmetric_right S T rS rT not_symT wS)
         end 
       | inr not_symS   => inr _ (brel_product_not_symmetric_left S T rS rT not_symS wT)
       end. 

Lemma brel_product_not_total : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
      brel_nontrivial S rS -> 
      brel_nontrivial T rT -> 
           brel_not_total (S * T) (brel_product S T rS rT). 
Proof. unfold brel_product, brel_not_total. 
       intros S T rS rT ntS ntT. 
       destruct(brel_nontrivial_pair S rS ntS) as [[s1 s2] [P1 P2]].
       destruct(brel_nontrivial_pair T rT ntT) as [[t1 t2] [Q1 Q2]].
       exists ((s1, t2), (s2, t1)). rewrite P1, P2. simpl. auto. 
Defined. 

Definition brel_product_total_decide : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
      brel_nontrivial S rS -> 
      brel_nontrivial T rT -> 
           brel_total_decidable (S * T) (brel_product S T rS rT)
:= λ S T rS rT ntS ntT,  inr _ (brel_product_not_total S T rS rT ntS ntT). 

Close Scope nat_scope. 

Lemma brel_total_split : 
   ∀ (S : Type) (eqS : brel S) (rS : brel S), 
      brel_total S rS -> 
      brel_nontrivial S eqS -> 
      brel_antisymmetric S eqS rS -> 
      {s1 : S & {s2 : S & (rS s2 s1 = true) * (rS s1 s2 = false) }}. 
Proof. intros S eqS rS totS ntS antiS. 
       unfold brel_antisymmetric in antiS. 
       destruct (brel_nontrivial_witness S eqS ntS) as [s Ps].
       destruct (brel_nontrivial_negate S eqS ntS) as [f Pf].
   ...
   ∀ (S T: Type) 
     ...
     (rT : brel T), 
      ...
      brel_total_decidable T rT -> 
           brel_total_decidable (S * T) (brel_product S T rS rT). 
Proof. 
  intros S T eqS eqT rS rT ntS ntT antiS antiT.
  ...
  intros [ totS | [ [s1 s2] [SL SR]]] [ totT | [ [t1 t2] [TL TR]]]. 
     destruct (brel_total_split S eqS rS totS ntS antiS) as [s1 [s2 [sP1 sP2]]]. 
     destruct (brel_total_split T eqT rT totT ntT antiT) as [t1 [t2 [tP1 tP2]]]. 
...
Lemma brel_product_asymmetric_left : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              (brel_asymmetric S rS) → 
                  brel_asymmetric (S * T) (brel_product S T rS rT). 
...
Lemma brel_product_not_asymmetric : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              ...
              (brel_not_asymmetric T rT) → 
                  brel_not_asymmetric (S * T) (brel_product S T rS rT). 
...
Definition brel_product_asymmetric_decide: 
   ∀ (S T: Type) 
     ...
     brel_asymmetric_decidable T rT → 
        brel_asymmetric_decidable (S * T) (brel_product S T rS rT)
:= λ S T rS rT dS dT,  
       match dS with 
...
Lemma brel_product_has_2_chain : 
         ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              ...
              (brel_has_2_chain T rT) → 
                  brel_has_2_chain (S * T) (brel_product S T rS rT). 
...
Lemma brel_product_not_has_2_chain_left : 
         ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              (brel_not_has_2_chain S rS) → 
                  brel_not_has_2_chain (S * T) (brel_product S T rS rT). 
...
Definition brel_product_has_2_chain_decide: 
   ∀ (S T: Type) 
     ...
     brel_has_2_chain_decidable T rT → 
        brel_has_2_chain_decidable (S * T) (brel_product S T rS rT)
:= λ S T rS rT dS dT,  
       match dS with 
       | inl h2cS       => 
         match dT with 
...
Lemma brel_product_transitive : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              (brel_transitive _ rS) → (brel_transitive _ rT) 
               → brel_transitive (S * T) (brel_product S T rS rT). 
Proof. 
     intros S T rS rT. unfold brel_transitive. intros RS RT [s1 t1] [s2 t2] [s3 t3]; simpl. 
     intros.           
     apply andb_is_true_right. 
     apply andb_is_true_left in H. 
...
Lemma brel_product_not_transitive_right : ∀ (S T: Type) (rS : brel S) (rT : brel T),  
              ...
              (brel_not_transitive T rT) → 
                  brel_not_transitive (S * T) (brel_product S T rS rT). 
...
Definition brel_product_transitive_decide : 
   ∀ (S T: Type) 
     ...
     brel_transitive_decidable T rT → 
        brel_transitive_decidable (S * T) (brel_product S T rS rT)
...
Definition brel_product_with_reflexivity_antisymmetric_decide : 
   ∀ (S T: Type) 
     ...
     brel_antisymmetric_decidable T eqT rT → 
        brel_antisymmetric_decidable (S * T) (brel_product S T eqS eqT) (brel_product S T rS rT)
