...
Assume $\propname{AY}(S,\ \leq)$. 
(Seems required for associativity, idempotence, perhaps others.)
...
\propname{ID}(\mathrm{MinUnion}(S, \leq)) 
  & \Leftrightarrow 
  & \propname{FINITE}(\min_\leq(S)) \ \ \ (\mbox{WHAT IS THIS REALLY? CLOSE W.R.T. ???})
  \\
...
\propname{SL}(\mathrm{MinLift}(S,\ \leq,\ \otimes)) 
  & \Leftrightarrow 
%% vilius : SL and (not SL or IAUS and IAF)? 
%% require \otimes to be selective??? 
%% IAUS : COMPLEX 
%% IAF  : COMPLEX 
  & ??? \ \ \ (\mbox{  SOMEDAY !!!})
  \\ 
\propname{IP}(\mathrm{MinLift}(S,\ \leq,\ \otimes)) 
  & \Leftrightarrow 
%% vilius : IP and (not IP or SND)? 
%% SND : x <= x . y or y <= x . y 
%% (not IP) or (exists x, y, not(x <= x . y) and not(y <= x . y))
%% case 1 : not IP: 
%%   exists a, a . a <> a. 
%%   A = {a} 
%% case 2 :  IP and exists x, y, not(x <= x . y) and not(y <= x . y))
%%   A = {x, y} 
%%   min(A . A) = min{x . x , y . y, x . y, y . x}
%%              = min{x, y, x . y, y . x} 
%%              <> {x, y} = A      
  & ??? \ \ \ (\mbox{  SOMEDAY !!!})
  \\ 
