...
$\propname{LM}(\mathrm{(S,\leq,\otimes)} \wedge \propname{RM}(\mathrm{(S,\leq,\otimes)} \Rightarrow \min_\leq(X \lift \min_\leq(Y)) = \min_\leq(X \lift Y) = \min_\leq(\min_\leq(X) \lift Y)$
