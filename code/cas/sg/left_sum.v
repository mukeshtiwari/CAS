...
Definition check_exists_id_left_sum : ∀ {S T : Type}, 
             (check_exists_id (S := T)) -> (check_exists_id (S := (S + T)))
:= λ {S T} cT,  
      match cT with 
...
Definition sg_certs_left_sum : ∀ {S T : Type},  eqv_certificates (S := S) -> eqv_certificates (S := T) -> sg_certificates (S := S) -> sg_certificates (S := T) -> sg_certificates (S := (S + T)) 
:= λ {S T} eS eT cS cT,  
...
Definition sg_C_certs_left_sum : ∀ {S T : Type},  eqv_certificates (S := S) -> eqv_certificates (S := T) -> sg_C_certificates (S := S) -> sg_C_certificates (S := T) -> sg_C_certificates (S := (S + T)) 
:= λ {S T} eS eT cS cT,  
let s := nontrivial_witness (eqv_nontrivial eS) in 
let f := nontrivial_negate (eqv_nontrivial eS) in 
let t := nontrivial_witness (eqv_nontrivial eT) in
...
Definition sg_CI_certs_left_sum : ∀ {S T : Type},  eqv_certificates (S := S) -> eqv_certificates (S := T) -> sg_CI_certificates (S := S) -> sg_CI_certificates (S := T) -> sg_CI_certificates (S := (S + T)) 
:= λ {S T} eS eT cS cT,  
{|
...
Definition sg_CS_certs_left_sum : ∀ {S T : Type},  eqv_certificates (S := S) -> eqv_certificates (S := T) -> sg_CS_certificates (S := S) -> sg_CS_certificates (S := T) -> sg_CS_certificates (S := (S + T)) 
:= λ {S T} eS eT cS cT,  
{|
...
:= λ {S T} sgS sgT, 
   {| 
     sg_CI_eqv       := eqv_sum 
                           (sg_CI_eqv sgS) 
                           (sg_CI_eqv sgT) 
   ; sg_CI_bop       := bop_left_sum 
                           (sg_CI_bop sgS) 
                           (sg_CI_bop sgT) 
   ; sg_CI_certs    := sg_CI_certs_left_sum 
...
:= λ {S T} sgS sgT, 
   {| 
     sg_CS_eqv       := eqv_sum 
                           (sg_CS_eqv sgS) 
                           (sg_CS_eqv sgT) 
   ; sg_CS_bop       := bop_left_sum 
                           (sg_CS_bop sgS) 
                           (sg_CS_bop sgT) 
   ; sg_CS_certs    := sg_CS_certs_left_sum 
